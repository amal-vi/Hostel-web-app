...
class Login(db.Model):
    ...
    user_type = db.Column(db.Enum('student', 'warden', 'admin'),nullable=False)

    student_info = db.relationship('Students', back_populates='login', uselist=False)
    warden_info = db.relationship('Wardens', back_populates='login', uselist=False)
...
class Wardens(db.Model):
    ...
    photo_url = db.Column(db.String(150),nullable=False)
    status = db.Column(db.Enum('inactive','active','pending'),nullable=False,default='pending')
    user_type = db.Column(db.String(150),nullable=False,default='warden')
    logid = db.Column(db.Integer,db.ForeignKey('login_info.logid'),nullable=False)

    login = db.relationship('Login', back_populates='warden_info')
